...
sweeps, GitHub Actions runner updates, or platform dependency compatibility
checks.

For `known-folders-rs`, also see:

- [`automations/windows-sys.md`](automations/windows-sys.md), for supported
  `windows-sys` range maintenance.

## Updating Docs

Update docs in the same PR when a change alters public behavior, compatibility
claims, target support, MSRV, dependency policy, release process, automation
