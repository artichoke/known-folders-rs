...
```sh
gh ruleset list --repo artichoke/known-folders-rs
gh api repos/artichoke/known-folders-rs/rulesets/<ruleset-id> \
...
```sh
gh api repos/artichoke/known-folders-rs/rulesets/<ruleset-id> \
  > /tmp/known-folders-rs-ruleset.json
jq '{name, target, enforcement, conditions, bypass_actors, rules}' \
  /tmp/known-folders-rs-ruleset.json > /tmp/known-folders-rs-ruleset.update.json
# Edit /tmp/known-folders-rs-ruleset.update.json so required check contexts
# match the workflow job names produced by the new matrix.
gh api -X PUT repos/artichoke/known-folders-rs/rulesets/<ruleset-id> \
  --input /tmp/known-folders-rs-ruleset.update.json
```
